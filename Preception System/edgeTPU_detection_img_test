...
class StandaloneSignDetector:
    ...
    def process_image(self, image_path_or_cv_image):
        ...
        if self.output_details[0]['dtype'] in [np.uint8, np.int8] and not (self.output_scale == 1.0 and self.output_zero_point == 0):
            ...
            # 這裡假設它需要被歸一化到 0-1 範圍 (YOLOv5 通常輸出0-1的信心度)
        ...
        # print(f"DEBUG: Raw dequantized output_data sample (first detection): {output_data_float[0][:10]}")
...
def main():
    MODEL_FILE_PATH = '/path/to/your/.tflite model'
    LABELS = [

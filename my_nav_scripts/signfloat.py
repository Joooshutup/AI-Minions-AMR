...
class SignDetectionNode(Node):
    def __init__(self):
        ...
        MODEL_PATH = '/home/garygay/autorace_ws/src/my_nav_scripts/weights/yolo_dynamic_quant.tflite'
        
        self.CONFIDENCE_THRESHOLD = 0.7 # 你已調整
        self.NMS_IOU_THRESHOLD = 0.4
        self.DRAW_OUTPUT = False # 保留本地顯示
        ...
        self.sign_pub = self.create_publisher(String, '/sign_detections', 10) # 文字發佈器保留
        self.bridge = CvBridge()
        self.last_frame_time = time.time()
        
        # --- CHANGE 1: Comment out the annotated image publisher ---
    ...
    def image_callback(self, msg: Image):
        ...
        model_expected_dtype = self.input_details[0]['dtype']
        input_data = None
        if model_expected_dtype == np.float32:
            input_data = input_tensor_untyped.astype(np.float32) / 255.0
        elif model_expected_dtype == np.uint8:
            input_data = input_tensor_untyped.astype(np.uint8)
        else:
            self.get_logger().error(f"Unsupported model input dtype: {model_expected_dtype}")
            return
        ...
        output_data = self.interpreter.get_tensor(self.output_tensor_index)[0]
        self.get_logger().info(f"PROFILE: Inference: {time.time() - inference_start_time:.4f}s")
        ...
        for i in range(output_data.shape[0]):
            ...
            if obj_score > self.CONFIDENCE_THRESHOLD: # CONFIDENCE_THRESHOLD 已設為 0.7
                class_scores = detection[5:]
                class_id = np.argmax(class_scores)
                confidence = obj_score * class_scores[class_id]
                if confidence > self.CONFIDENCE_THRESHOLD: # 再次檢查（通常YOLO輸出obj_score就是最終的）
        ...
        image_to_annotate = cv_image.copy() # 仍然創建副本用於本地顯示

        draw_publish_start_time = time.time()
        if len(indices) > 0:
            ...
            for i in processed_indices:
                ...
                if class_id < len(self.labels):
                    ...
                    # 繪圖操作保留，因為 self.DRAW_OUTPUT = True
                    cv2.rectangle(image_to_annotate, (x_b, y_b), (x_b + w_b, y_b + h_b), (0, 255, 0), 2)
                    text = f"{label_name}: {confidence_val:.2f}"
                    cv2.putText(image_to_annotate, text, (x_b, y_b - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                else:
        ...
        if detected_objects_this_frame:
            ...
            for obj in unique_detected_objects: self.sign_pub.publish(String(data=obj)) # 文字發佈保留

        if self.DRAW_OUTPUT: # 本地顯示保留
            cv2.putText(image_to_annotate, f"FPS: {fps:.1f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
            cv2.imshow("TFLite Detection (ROS 2)", image_to_annotate)
            cv2.waitKey(1)

        self.get_logger().info(f"PROFILE: Draw & Publish: {time.time() - draw_publish_start_time:.4f}s") # 這個時間現在主要反映繪圖和imshow的耗時
...
def main(args=None):
    ...
    try:
        ...
        if node and hasattr(node, 'destroy_node') and callable(node.destroy_node):
            ...
            except Exception as e: print(f"Error destroying node: {e}")
        if rclpy.ok(): rclpy.shutdown()

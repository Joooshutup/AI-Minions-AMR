...
class SingleLineTracker(Node):
    def __init__(self):
        ...
        self.offset_yellow = 210  
        # 白線在右側，所以道路中心 = 白線中心 - offset
        self.offset_white = 210
    ...
    def do_detect_park_dir(self):
        ...
        left_range  = np.nanmin(valid_ranges[mid_index + 30:mid_index + 90])
        right_range = np.nanmin(valid_ranges[mid_index - 90:mid_index - 30])
        ...
        if right_range > left_range:  # 左邊比較近 → 右轉
...
# #########################################################
    ...
    def do_line_tracking(self):
        ...
        if road_center_x is not None:
            ...
            linear_speed = 0.07  # 可維持固定或依 error 動態調整
            # 基本的比例控制 (P control)

...
import cv2
import numpy as np
import time # time 模塊仍然用於 callback_overall_start_time 等，但其日誌輸出被註釋
...
class SignDetectionNode(Node):
    def __init__(self):
        ...
        MODEL_PATH = '/home/garygay/autorace_ws/src/my_nav_scripts/weights/thebest.tflite'
        
        self.CONFIDENCE_THRESHOLD = 0.7 
        self.NMS_IOU_THRESHOLD = 0.4
        self.DRAW_OUTPUT = False # 保持為 False，以專注於非GUI性能和檢測結果日誌

        self.labels = [
            "Obstacle", "Gate Down", "Gate Up", "Green Light", "Left", "No Entry",
            "Parking", "Red Light", "Right", "Stop", "T", "Tunnel", "Yellow Light"
        ]
        self.get_logger().info(f"Using {len(self.labels)} hardcoded labels: {self.labels}") # 保留這個有用的信息
        ...
        try:
            ...
            # --- DETAILED OUTPUT TENSOR INFO BLOCK - NOW COMMENTED OUT or set to DEBUG ---
            ...
            self.get_logger().debug(f"DEBUG: Raw model input shape from TFLite: {model_input_shape}") # Changed to DEBUG
        ...
        self.get_logger().info("---辨識節點初始化了---") # 保留這個重要的初始化成功信息
        ...
        if len(indices) > 0:
            ...
            for i in processed_indices:
                ...
                if class_id < len(self.labels): 
                    ...
                    self.get_logger().warn(f"Detected class_id {class_id} out of bounds for labels list (len: {len(self.labels)}).") # 保留警告
